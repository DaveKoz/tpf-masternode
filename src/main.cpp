...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    //////////////////////////////////////
    if (nHeight >= 199) {
        ret = blockValue * 0.2; // reward ~MN 20% - Pos 20% after blk 100
    }
    //////////////////////////////////////
    return ret;
}
...
bool IsInitialBlockDownload()
{
    ...
    if (lockIBDState)
        return false;
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 || pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60);
    if (!state)
        lockIBDState = true;
    return state;
}
...
void CheckForkWarningConditions()
{
    ...
    // If our best fork is no longer within 72 blocks (+/- 6 hours if no one mines it)
    // of our head, drop it
    if (pindexBestForkTip && chainActive.Height() - pindexBestForkTip->nHeight >= 72)
        pindexBestForkTip = NULL;

    if (pindexBestForkTip || (pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (GetBlockProof(*chainActive.Tip()) * 12))) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || !((pindex->nHeight == 91842 && pindex->GetBlockHash() == uint256("0x00000000000a4d0a398161ffc163c503763b1f4360639393e0e4c8e300e0caec")) ||
